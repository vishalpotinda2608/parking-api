...
function getNodes(definitionNodes, options) {
    ...
    const nodes = definitionNodes.objectTypes.map((definition) => {
        const otherDirectives = (definition.directives || []).filter((x) => !["auth", "exclude", "node", "fulltext", "queryOptions", "plural"].includes(x.name.value));
        ...
        nodeFields.relationFields.forEach((relationship) => {
            if (relationship.properties) {
                const propertiesInterface = definitionNodes.interfaceTypes.find((i) => i.name.value === relationship.properties);
        ...
        if (globalIdFields.length > 1) {
            throw new Error("Only one field may be decorated with an '@id' directive with the global argument set to `true`");
        }
        ...
        if (globalIdField && idField) {
            ...
            if (!hasAlias) {
                throw new Error(`Type ${definition.name.value} already has a field "id." Either remove it, or if you need access to this property, consider using the "@alias" directive to access it via another field`);
            }
        }
        if (globalIdField && !globalIdField.unique) {
            throw new Error(`Fields decorated with the "@id" directive must be unique in the database. Please remove it, or consider making the field unique`);
        }

            ...
            refNodes.forEach((refNode, idx) => {
                ...
                updates.forEach((update, index) => {
                    ...
                    if (update.update) {
                        ...
                        subquery.push(`OPTIONAL MATCH (${parentVar})${inStr}${relTypeStr}${outStr}(${variableName}${labels})`);
                        ...
                        if (update.update.node) {
                            ...
                            const apocArgs = `{${withVars.map((withVar) => `${withVar}:${withVar}`).join(", ")}, ${parameterPrefix?.split(".")[0]}: $${parameterPrefix?.split(".")[0]}, ${variableName}:${variableName}REPLACE_ME}`;
                        ...
                        if (update.update.edge) {
                            ...
                            const updateStrs = [(0, escape_query_1.escapeQuery)(setProperties), (0, escape_query_1.escapeQuery)("RETURN count(*) AS _")];
                            const varsAsArgumentString = withVars
                                .map((variable) => `${variable}:${variable}`)
                                .join(", ");
                            const apocArgs = `{${varsAsArgumentString}, ${relationshipVariable}:${relationshipVariable}, ${parameterPrefix?.split(".")[0]}: $${parameterPrefix?.split(".")[0]}, resolvedCallbacks: $resolvedCallbacks}`;

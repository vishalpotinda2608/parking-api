...
function createDisconnectAndParams({ withVars, value, varName, relationField, parentVar, refNodes, context, labelOverride, parentNode, insideDoWhen, parameterPrefix, isFirstLevel = true, }) {
    function createSubqueryContents(relatedNode, disconnect, index) {
        ...
        subquery.push(`}`);
        if (context.subscriptionsEnabled) {
            subquery.push(`WITH ${(0, filter_meta_variable_1.filterMetaVariable)(withVars).join(", ")}, ${variableName}, meta + update_meta as meta`);
        }
    ...
    function reducer(res, disconnect, index) {
        ...
        if (relationField.interface) {
            ...
            if (subqueries.length > 0) {
                if (context.subscriptionsEnabled) {
                    const withStatement = `WITH ${(0, filter_meta_variable_1.filterMetaVariable)(withVars).join(", ")}, disconnect_meta + meta AS meta`;
        ...
        if (inner.length > 0) {
            ...
            if (context.subscriptionsEnabled) {
                res.disconnects.push(`WITH ${(0, filter_meta_variable_1.filterMetaVariable)(withVars).join(", ")}, disconnect_meta + meta as meta`);
            }
        }
        return res;
    }
